...
__global__ void bilagrid_uniform_sample_backward_v1_kernel_bilagrid(
    ...
    int block_wi0 =
        max((int)ceil((xi - 1) * sw), 0); // same for all threads in the block
        ...
        for (int mi = 0; mi < m; ++mi) {
            for (int wi = wi0; wi < wi1; wi++)
                for (int hi = hi0; hi < hi1; hi++) {
                    ...
                    if (zi == z0) {
                        if (xi == x0 && yi == y0)
                            accum_t += (1 - fx) * (1 - fy) * (1 - fz);
                        if (xi == x1 && yi == y0)
                            accum_t += fx * (1 - fy) * (1 - fz);
                        if (xi == x0 && yi == y1)
                            accum_t += (1 - fx) * fy * (1 - fz);
                        if (xi == x1 && yi == y1)
                            accum_t += fx * fy * (1 - fz);
                    }
                    if (zi == z1) {
                        if (xi == x0 && yi == y0)
                            accum_t += (1 - fx) * (1 - fy) * fz;
                        if (xi == x1 && yi == y0)
                            accum_t += fx * (1 - fy) * fz;
                        if (xi == x0 && yi == y1)
                            accum_t += (1 - fx) * fy * fz;
...
#pragma unroll
        for (int di = 0; di < 3; di++) {
            ...
            float val = bilagrid[base + (z0 * H + y0) * W + x0] * w000 +
...
void bilagrid_uniform_sample_backward_v1(
    ...
    // v_bilagrid
    {
        ...
        dim3 bounds = {
            (unsigned int)((W * mult_x + block.x - 1) / block.x),
            (unsigned int)((H * mult_y + block.y - 1) / block.y),
            (unsigned int)((N * L + block.z - 1) / block.z)
        };
        bilagrid_uniform_sample_backward_v1_kernel_bilagrid<<<bounds, block>>>(
            rgb, v_output, v_bilagrid, N, L, H, W, m, h, w, mult_x, mult_y
        );
    }
    ...
    // v_coords and v_rgb
    {
        ...
        bilagrid_uniform_sample_backward_v1_kernel_rgb<<<blocks, threads>>>(
            bilagrid, rgb, v_output, v_rgb, N, L, H, W, m, h, w
        );
    }
}